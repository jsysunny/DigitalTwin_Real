...
class LaneFollower(Node):
    def __init__(self):
        ...
        self.cmd_pub = self.create_publisher(Twist, '/cmd_vel', 10)
        self.debug_frame_pub = self.create_publisher(Image, '/lane/debug_frame', 10)
        self.create_subscription(MarkerArray, '/detected_markers', self.marker_callback, 10)
        # pick_and_place trigger
        self.pick_pub = self.create_publisher(MarkerArray, '/pick_trigger', 10)
        self.marker_detected = False
        # joint_states + arm command
        ...
        self.joint1_offset = 1.0
        self.arm_state = 'restored'
    ...
    def move_joint1(self, new_pos):
        self.current_joints[0] = new_pos
        traj = JointTrajectory()
        traj.joint_names = self.joint_names
        p = JointTrajectoryPoint()
        p.positions = list(self.current_joints)
        p.velocities = [0.0]*4
    ...
    def image_callback(self, msg):
        ...
        # masks
        y_mask = cv2.inRange(hsv, (15, 40, 100), (70, 255, 255))
        w_mask = cv2.inRange(hsv, (0, 0, 180), (180, 60, 255))
        num_labels_white, _ = cv2.connectedComponents(w_mask)
        num_labels_yellow, _ = cv2.connectedComponents(y_mask)

        y_cnt, w_cnt = cv2.countNonZero(y_mask), cv2.countNonZero(w_mask)
        both = y_cnt > 150 and w_cnt > 150
        only_y = y_cnt > 150 and w_cnt <= 150
        only_w = w_cnt > 150 and y_cnt <= 150

        # joint1 조절
        if only_y and self.arm_state not in ['right', 'locked']:
            self.move_joint1(+self.joint1_offset)
            self.arm_state = 'right'
        elif only_w and self.arm_state not in ['left', 'locked']:
            self.move_joint1(-self.joint1_offset)
            self.arm_state = 'left'
        elif both and self.arm_state in ['right', 'left']:
            self.move_joint1(0.0)
            self.arm_state = 'locked'
