...
class ArucoMarkerDetector(Node):
    def __init__(self):
        ...
        self.marker_image_pub = self.create_publisher(Image, '/aruco/markers/image_raw', 10)
    ...
    def listener_callback(self, msg):
        ...
        if len(detect_data) == 0:
            ...
            for marker in detect_data:
                ...
                f"[ID {marker[0][0]}] Pos: ({marker[1][0]:.2f}, {marker[1][1]:.2f}, {marker[1][2]:.2f}), "
                f"Euler (yaw={marker[2][0]:.1f}, pitch={marker[2][1]:.1f}, roll={marker[2][2]:.1f}), "
                f"Dist: {marker[3]:.2f} m"
                )
        ...
        self.marker_image_pub.publish(ros_img)
        #cv2.imshow('Detected Markers', frame)
        #cv2.waitKey(1)
